...
<xsl:stylesheet exclude-result-prefixes="#all" version="3.0" xmlns="http://www.tei-c.org/ns/1.0" xmlns:bib="http://purl.org/net/biblio#" xmlns:dc="http://purl.org/dc/elements/1.1/"
    ...
    <xsl:function name="oape:string-convert-lang-codes">
        ...
        <xsl:if test="$p_input = 'und'">
            <xsl:message>Input language not supported</xsl:message>
        </xsl:if>
        <xsl:if test="$p_input != 'und'">
            ...
            <xsl:choose>
                ...
                <xsl:when test="$p_source-encoding = 'bcp47' and $p_target-encoding = 'iso639-2'">
                    <xsl:value-of select="replace($v_iso639-2_bcp47, concat('^.*;(\w{3})=', $p_input, ';.*$'), '$1')"/>
                </xsl:when>
                <xsl:when test="$p_source-encoding = 'bcp47' and $p_target-encoding = 'wikidata'">
                ...
                <xsl:otherwise>
                    ...
                    <xsl:message>
                        <xsl:text>Combination of language encodings not supported</xsl:text>
                    </xsl:message>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:if>
