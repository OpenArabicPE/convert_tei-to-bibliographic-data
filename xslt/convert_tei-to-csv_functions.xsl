...
<!--    <xsl:include href="../../tools/xslt/openarabicpe_functions.xsl"/>-->
    ...
    <xsl:param name="p_file-name-bibliography" select="'bibliography_OpenArabicPE-periodicals.TEIP5.xml'"/>
...
<!--        <xsl:param name="p_lang"/>-->
        ...
        <xsl:variable name="v_volume">
            <xsl:apply-templates  select="$p_input/tei:monogr/tei:biblScope[@unit = 'volume']" mode="m_tei-to-csv"/>
        </xsl:variable>
        <xsl:variable name="v_issue">
          <xsl:apply-templates  select="$p_input/tei:monogr/tei:biblScope[@unit = 'issue']" mode="m_tei-to-csv"/>
        </xsl:variable>
        <xsl:variable name="v_page-count">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="$p_input//tei:biblScope[@unit = 'page']/@to - $p_input//tei:biblScope[@unit = 'page']/@from +1"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:template  match="tei:persName" mode="m_tei-to-csv">
        <xsl:choose>
            <xsl:when test="tei:surname and tei:forename">
               <xsl:apply-templates select="tei:surname" mode="m_tei-to-csv"/>
                <xsl:text>, </xsl:text>
                <xsl:apply-templates select="tei:forename" mode="m_tei-to-csv"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:apply-templates select="." mode="m_plain-text"/>
            </xsl:otherwise>
